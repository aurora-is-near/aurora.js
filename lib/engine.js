...
import { DeployErc20TokenArgs, ExecutionResult, FunctionCallArgs, GetStorageAtArgs, InitCallArgs, NewCallArgs, ViewCallArgs, } from './schema.js';
...
export class Engine {
    ...
    async deploy_erc20_token(nep141) {
        const args = new DeployErc20TokenArgs(nep141);
        return (await this.callMutativeFunction('deploy_erc20_token', args.encode())).map(({ output }) => output);
    }
