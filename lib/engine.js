...
import { ExecutionResult, FunctionCallArgs, GetStorageAtArgs, InitCallArgs, NewCallArgs, ViewCallArgs, } from './schema.js';
...
export class Engine {
    ...
    async initialize(options) {
        ...
        const connectorArgs = new InitCallArgs(options.prover || 'prover.ropsten.testnet', options.ethCustodian || '9006a6D7d08A388Eeea0112cc1b6b6B15a4289AF');
        ...
        let tx = await this.promiseAndThen(this.callMutativeFunction('new', newArgs.encode()), (_) => this.callMutativeFunction('new_eth_connector', connectorArgs.encode()));
        return tx.map(({ id }) => id);
    }
    ...
    async submit(input) {
        try {
            ...
            return (await this.callMutativeFunction('submit', inputBytes)).map(({ output }) => ExecutionResult.decode(Buffer.from(output)));
        }
        catch (error) {
            //console.error(error); // DEBUG
            return Err(error.message);
